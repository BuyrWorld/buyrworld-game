...
- [x] **M20 — Logistics Decisions** · `data/logistics.ts` — choose HOW to ship each contract, trading cost vs payout vs risk: ⚡ Express (a courier's cut, but guaranteed), 🚚 Standard (full pay, minor hiccup chance), 🛳️ Freight (a bonus you keep, but a real delay/damage risk). Resolved instantly at delivery via `resolveShipment()` (keeps the instant "+coins" feel); EV ordered Freight>Standard>Express. Shared method picker on the Contracts board (default in `S.logistics.method`); card payouts reflect the choice; a Freight mishap is visible (toast + log + newspaper headline). Guarded by `tests/logistics.test.ts`.
...
- [x] **M24 — Town Evolution** · `data/townEvolution.ts` — Featherstone visibly grows with the player's **net worth** through 5 stages (Hamlet → Village → Market Town → Bustling Town → Boomtown). Crossing a threshold fires a one-time celebration (toast + log + newspaper headline); the Town Directory shows the current stage + progress to the next; and ambient world props unlock per tier (bunting, extra market stalls, banners, night street-lamps — decorative/non-colliding, reduced-motion-safe). Old saves default seenTier to their current tier (no false level-up). Guarded by `tests/townEvolution.test.ts`.
